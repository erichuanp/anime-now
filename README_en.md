...
No account, no server. Requests go from your device straight to Bangumi, Tavily and the provider you choose. Settings stay in the app's own directory; API keys are encrypted and bound to the device. Nothing is collected.

The encryption key is derived from a device identifier. It protects a config.json copied off the device on its own; it does not protect against someone who can read the whole machine. Do not treat it as a vault.

Using the Anime Now mirror means agreeing that the keywords you search and the bangumi usernames you enter pass through the author's server, where the author can see them. Your IP is used only to count the daily quota: the server stores a salted hash of it rather than the address, and the access log records no client address. Switch back to the official source, or point the app at your own mirror, to avoid this. Tavily and LLM requests never go through the mirror; ask those providers about their own privacy terms.

## Download

[Releases](https://github.com/erichuanp/anime-now/releases). Android 6.0+, most phones want `arm64-v8a`; macOS 12+ takes the `.dmg`.

The macOS build is unsigned: right-click the icon and choose Open the first time.
