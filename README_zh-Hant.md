...
加密用的金鑰由裝置識別碼派生，擋的是「config.json 被單獨拷走」；能讀你整台裝置的人一樣解得開，別把它當保險箱。

使用番時鏡像即表示同意：查番時輸入的關鍵字和 bangumi 使用者名稱會經過作者的伺服器，作者可以看到。你的 IP 只用來算每日配額，伺服器存的是加鹽雜湊而不是位址本身，存取記錄裡也不記錄用戶端位址。換回官方或填自己的鏡像即可避免。Tavily 和 LLM 的請求不經過鏡像，它們的隱私問題請詢問對應的服務商。
